...
# (date, circuit, session, note). session: 'am_pm' = split 10-14 / 15-18,
# 'pm' = afternoon-only (sunset events).
EVENTS: list[tuple[date, str, str, str]] = [
    (date(2026, 8, 29), "Andalucia Circuit",    "pm",    "Sunset — open pitlane 17:00–21:00"),
    (date(2026, 10, 10), "Andalucia Circuit",   "am_pm", "Open pitlane 10:00–14:00 / 15:00–18:00"),
    (date(2026, 11, 14), "Look and Run Circuit", "am_pm", "Open pitlane 10:00–14:00 / 15:00–18:00"),
    (date(2026, 12, 19), "Andalucia Circuit",   "am_pm", "Open pitlane 10:00–14:00 / 15:00–18:00"),
]
...
async def fetch() -> list[RawEvent]:
    today = date.today()
    out: list[RawEvent] = []
    for event_date, circuit, session, note in EVENTS:
        if event_date < today:
            continue
        out.append(RawEvent(
            source=SOURCE_SLUG,
